...
export const initializeOwner = (configService: ConfigService): void => {
  const secretKey = configService.get<string>('SERVICE');
  if (!secretKey) {
    throw new Error('SERVICE environment variable is not defined');
  }
  owner = Keypair.fromSecretKey(bs58.decode(secretKey));
};
